...
const opts = {
    ...
    fileNamePattern: 'Destroy-<DATE>.log',
...
const check_valid_request = (req) => {
    if (String(req.url).includes('dns.google.com')) {
        return true;
    }
    var skippedResources = [
        ...
        'gravatar.com',
        'af.opera.com',
        'id.google.com', 'adservice.google.com', 'features.opera-api.com', 'mtalk.google.com', 'exchange.opera.com', 'autoupdate.geo.opera.com',
        'redirector.gvt1.com',
        'gvt1.com',
        'facebook.com'
    ];
    for (let _url of skippedResources) {
    	if (String(req.url).includes(_url)) {
    		// console.log('DESTROY ==>: ', req.url);
    		log.info('DESTROY ==>: ', req.url);
    		return false;
    	}
    }
    // console.error('------ PASS: ', req.url);
    return true;
};
...
var Server = exports.Server = function (_EventEmitter) {
    ...
    _createClass(Server, [{
        ...
        value: function onRequest(request, response) {
            if (false === check_valid_request(request)) {
